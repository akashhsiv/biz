...
/// The session token is a bearer credential, so it lives in secure storage (Windows Credential
/// Manager–backed). The profile cache lets the app restore its UI (name/role/permissions) without
/// a network call on launch; if the cached token turns out to be expired, the first real API call
/// will 401 and the app falls back to the login screen.
class AppStorage {
